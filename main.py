...
async def rag_ingest_pdf(ctx: inngest.Context):
    ...
    def _upsert(chunks_and_src: RAGChunkAndSrc) -> RAGUpsertResult:
        ...
        QdrantStorage.upsert(vectors=vecs, ids=ids, payloads=payloads)
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    ...
    question = ctx.event_data["question"]
    top_k = ctx.event_data.get("top_k", 5)
    ...
    user_content = (
        ...
        f"Context:\n{context_block}\n\n"
        f"Question: {question}\n\n"
        "Asnwer the question based on the context provided"
    )
    adapter = ai.OpenAIAdapter(api_key=os.getenv("OPENAI_API_KEY"), 
                               model="gpt-4o-mini")
    res = await ctx.step.ai.infer(
        ...
        body={
            ...
            "temperature":0.2,
            "messages":[
                {"role":"system", "content":"You are a helpful assistant that helps answer questions based on provided context."},
